...
def write_report():
    # Priority classifier with associated headers
    priority_buckets = {
        "Critical Security": [],
        "System Integrity": [],
    ...
    for rec in recommendations:
        rec_lower = rec.lower()
        if any(k in rec_lower for k in ["firewall", "auditd", "rootkit"]):
            priority_buckets["Critical Security"].append(rec)
        elif any(k in rec_lower for k in ["passwd", "shadow", "empty password", "password policy"]):
            priority_buckets["System Integrity"].append(rec)
        elif any(k in rec_lower for k in ["ssh", "open ports", "sudoers"]):
            priority_buckets["Network & Access Control"].append(rec)
        elif any(k in rec_lower for k in ["cron", "user", "nopasswd"]):
            priority_buckets["Service & User Management"].append(rec)
        elif any(k in rec_lower for k in ["update", "world-writable"]):
            priority_buckets["System Configuration"].append(rec)
        else:
            priority_buckets["Other"].append(rec)

    with open("audit.txt", "w") as f:
        f.write(f"Linux Audit Report - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write("\n\n".join(report))

        if any(priority_buckets.values()):
            f.write("\n\n🛠️ Recommendations (by Priority):\n")
            for section, recs in priority_buckets.items():
                if recs:
                    f.write(f"\n🔷 {section}:\n")
                    for rec in recs:
                        f.write(f"- {rec}\n")
